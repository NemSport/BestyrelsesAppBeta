...
function fromRole(role: RoleProfileView): RoleDraft {
  return {
    ...
    onboarding: {
      ...
      practicalInformation: role.onboardingGuide?.practical_information ?? "",
    },
  };
}
...
function addResponsibilityAreaInput(
  ...
  const existing = areas.find(
    (area) => normalizeAreaName(area.name) === normalized,
  );
  if (existing) {
    return {
      ...draft,
...
function compactFieldErrors(payload: {
...
export function JobCardRegister({
  ...
  const [statusFilter, setStatusFilter] = useState<JobCardStatusFilter>("all");
  ...
  const filteredRoles = data.roles.filter((role) => {
    ...
    return (
      matchesSearch && matchesStatus && matchesCommittee && matchesAssignment
    );
  });
    ...
    try {
      const response = await fetch(
          ...
          : `/api/organizations/${organizationId}/job-cards`,
        {
          ...
          body: JSON.stringify(
            payloadForSubmit(organizationId, draftForSubmit),
          ),
        },
      );
  ...
  function createArea() {
    ...
    setDraft(
      addResponsibilityAreaInput(draft, areaName, data.responsibilityAreas),
    );
    setAreaName("");
  }
  ...
  async function suggest(role?: RoleProfileView) {
    ...
    const normalizedAreas = new Set(
      suggestion.responsibilityAreas.map((value) => normalizeAreaName(value)),
    );
    ...
    setDraft({
      ...
      responsibilityAreaIds: data.responsibilityAreas
        .filter((area) => normalizedAreas.has(normalizeAreaName(area.name)))
        .map((area) => area.id),
      responsibilityAreaNames: suggestion.responsibilityAreas.filter(
        (suggestedArea) =>
          !data.responsibilityAreas.some(
            (area) =>
              normalizeAreaName(area.name) === normalizeAreaName(suggestedArea),
          ),
      ),
      ...
      taskTemplates: suggestion.taskTemplates.flatMap((template) => {
        ...
        return committee ? [{ ...template, committeeId: committee.id }] : [];
      }),
  ...
  return (
    <div className="content-flow">
      ...
      {data.canManage ? (
        ...
        <section className="workflow-panel border-line-strong bg-subtle/45">
          <p className="font-semibold">Organisationens rollehåndbog</p>
          <p className="mt-1 max-w-3xl text-sm leading-6 text-muted">
            Du kan læse jobkort, onboarding og hente PDF. Oprettelse og
            redigering håndteres af organisationens ejere og administratorer.
          </p>
        </section>
      )}
      {taskMessage ? (
      ...
      {pdfError ? (
        <div className="alert-danger p-3 text-sm">{pdfError}</div>
      ) : null}
      {data.roles.length ? (
        <>
          <section className="module-filter-surface">
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-[minmax(220px,1fr)_auto_auto_auto] lg:items-end">
            ...
            <div className="register-summary-bar mt-3">
          ...
          {filteredRoles.length ? (
            <div className="space-y-4">
              {filteredRoles.map((role) => (
                <article
                  className="entity-record px-4 py-5 sm:px-5"
                  id={`job-card-${role.id}`}
                  key={role.id}
                >
                  <div className="entity-header">
                    <div className="min-w-0 flex-1">
                      <div className="status-cluster">
                        ...
                        <StatusBadge
                          tone={role.archived_at ? "neutral" : "success"}
                        >
                          {role.archived_at ? "Arkiveret" : "Aktiv"}
                        </StatusBadge>
                      </div>
                      <h2 className="entity-title mt-1 text-lg">
                        {role.title}
                      </h2>
                      <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <div className="mt-5 grid gap-px overflow-hidden border border-line bg-line sm:grid-cols-2 xl:grid-cols-4">
                    <JobCardScanBlock
                      index="1"
                      title="Formål"
                      value={role.purpose || role.description}
                    />
                    <JobCardScanBlock
                      index="2"
                      title="Ansvar"
                    ...
                    <JobCardScanBlock
                      ...
                      title="Næste skridt"
                      value={
                        role.onboardingGuide?.first_30_days ||
                        role.relatedTasks.find(
                          (task) =>
                            !task.archived_at &&
                  ...
                  <div className="mt-4 grid gap-4 border-y border-line py-4 sm:grid-cols-2 lg:grid-cols-3">
                    <CompactMetaList
                      empty="Ingen er tilknyttet rollen."
                      items={role.assignments.map((assignment) => ({
                        id: assignment.id,
                    ...
                    />
                    <CompactMetaList
                      empty="Ingen udvalg er tilknyttet."
                      items={role.committees.map((committee) => ({
                        id: committee.id,
                    ...
                    />
                    <CompactMetaList
                      empty="Ingen ansvarsområder er angivet."
                      items={role.responsibilityAreas.map((area) => ({
                        id: area.id,
                        label: area.name,
                      }))}
                  ...
                  <details className="mt-5 border-t border-line pt-4">
                    <summary className="min-h-11 cursor-pointer py-2 text-sm font-semibold text-brand hover:underline">
                      Se hele jobkortet og onboarding
                    </summary>
                    <div className="mt-4 grid gap-5 lg:grid-cols-2">
                      <TextBlock
                        title="Kort rollebeskrivelse"
                        value={role.description}
                      />
                      <TextBlock
                        title="Onboardingintroduktion"
                        value={role.onboardingGuide?.introduction}
                      />
                      <TextBlock
                        title="Næste skridt · de første 30 dage"
                      ...
                      <TextBlock
                        title="Ikke ansvar for"
                        value={role.exclusions}
                      />
                      <TextBlock
                        title="Kompetencer"
                        value={role.competencies}
                      />
                      <TextBlock
                        title="Samarbejde"
                        value={role.collaboration}
                      />
                      <TextBlock
                        ...
                        {role.taskTemplates.length ? (
                          <div className="mt-2 space-y-2">
                            {role.taskTemplates.map((template) => (
                              <div
                                className="flex flex-wrap items-center justify-between gap-3 border-b border-line py-2"
                                key={template.id}
                              >
                        ...
                        {role.documents.length ? (
                          <div className="mt-2 space-y-2">
                            {role.documents.map((document) => (
                              <a
                                className="block min-h-11 break-words py-2 text-sm font-medium text-brand underline-offset-4 hover:underline"
                                href={document.url}
                                key={document.id}
                                rel="noreferrer"
                                target="_blank"
...
function JobCardScanBlock({
  ...
  return (
    <section className="min-w-0 bg-surface px-4 py-4">
      ...
      <RichTextContent
        className="mt-2 line-clamp-4 break-words text-sm leading-6"
        emptyText={
          title === "Næste skridt"
            ? "Ingen næste skridt er beskrevet endnu."
            : `${title} er ikke beskrevet endnu.`
        }
        value={value}
      />
    </section>
    ...
    <section className="min-w-0">
      ...
      {items.length ? (
        <div className="mt-2 space-y-1">
          {items.map((item) => (
            <p className="break-words text-sm" key={item.id}>
              {item.label}
              {item.detail ? (
  ...
  return (
    <section className="min-w-0">
      ...
      <RichTextContent
        ...
        emptyText="Ikke beskrevet endnu."
        value={value}
      />
    </section>
  );
    ...
    <section>
      ...
      {items.length ? (
        <div className="mt-2 space-y-2">
          {items.map((item) => (
            <Link
              className="block min-h-11 break-words py-2 text-sm text-brand underline-offset-4 hover:underline"
              href={item.href}
              key={item.id}
            >
              {item.title}
...
function RoleModal(props: {
  ...
  return (
    <Modal
      ...
      open={Boolean(draft)}
      title={draft?.id ? "Rediger jobkort" : "Opret jobkort"}
    >
      {draft ? (
        <form className="space-y-6" onSubmit={props.onSubmit}>
          {props.error ? (
            <div className="alert-danger p-3 text-sm">{props.error}</div>
          ) : null}
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Titel">
              <Input
                value={draft.title}
                onChange={(e) =>
                  props.onDraft({ ...draft, title: e.target.value })
                }
              />
            </Field>
            <Field label="Formål">
              <Input
                value={draft.purpose}
                onChange={(e) =>
                  props.onDraft({ ...draft, purpose: e.target.value })
                }
              />
            </Field>
          </div>
          <Field label="Beskrivelse">
            <Textarea
              ...
              onChange={(e) =>
                props.onDraft({ ...draft, description: e.target.value })
              }
            />
          </Field>
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="Hvad rollen har ansvar for">
              <Textarea
                value={draft.responsibilities}
                onChange={(e) =>
                  props.onDraft({ ...draft, responsibilities: e.target.value })
                }
              />
            </Field>
            <Field label="Hvad rollen ikke har ansvar for">
              <Textarea
                value={draft.exclusions}
                onChange={(e) =>
                  props.onDraft({ ...draft, exclusions: e.target.value })
                }
              />
            </Field>
            <Field label="Kompetencer">
              <Textarea
                ...
                onChange={(e) =>
                  props.onDraft({ ...draft, competencies: e.target.value })
                }
              />
            </Field>
            <Field label="Samarbejder med">
              <Textarea
                value={draft.collaboration}
                onChange={(e) =>
                  props.onDraft({ ...draft, collaboration: e.target.value })
                }
              />
            </Field>
            <Field label="Normale møder">
              <Textarea
                value={draft.meetingExpectations}
                onChange={(e) =>
                  props.onDraft({
                    ...draft,
                    meetingExpectations: e.target.value,
                  })
                }
              />
            </Field>
            <Field label="Kontaktpersoner">
              <Textarea
                value={draft.contactPeople}
                onChange={(e) =>
                  props.onDraft({ ...draft, contactPeople: e.target.value })
                }
              />
            </Field>
          </div>
          <ChoiceGroup
            ...
            options={data.responsibilityAreas.map((area) => ({
              id: area.id,
              label: area.name,
            }))}
            values={draft.responsibilityAreaIds}
            onToggle={(id) =>
              props.onDraft({
                ...draft,
                responsibilityAreaIds: toggle(draft.responsibilityAreaIds, id),
              })
            }
          />
                ...
                <button
                  ...
                  onClick={() =>
                    props.onDraft({
                      ...draft,
                      responsibilityAreaNames:
                        draft.responsibilityAreaNames.filter(
                          (current) =>
                            normalizeAreaName(current) !==
          ...
          <div className="flex gap-2">
            <Input
              list="job-card-responsibility-areas"
              placeholder="Skriv nyt eller eksisterende ansvarsområde"
              value={props.areaName}
              onChange={(e) => props.onAreaName(e.target.value)}
            />
            <datalist id="job-card-responsibility-areas">
              {data.responsibilityAreas.map((area) => (
                <option key={area.id} value={area.name} />
              ))}
            </datalist>
            <Button
              disabled={!props.areaName.trim()}
          ...
          <ChoiceGroup
            label="Udvalg"
            options={data.committees.map((committee) => ({
              id: committee.id,
              label: committee.name,
            }))}
            values={draft.committeeIds}
            onToggle={(id) =>
              props.onDraft({
                ...draft,
                committeeIds: toggle(draft.committeeIds, id),
              })
            }
          />
          <ChoiceGroup
            ...
            values={draft.assignedUserIds}
            onToggle={(id) =>
              props.onDraft({
                ...draft,
                assignedUserIds: toggle(draft.assignedUserIds, id),
              })
            }
          />
            ...
            <div className="mt-4 space-y-5">
              <RelationChoiceGroup
                ...
                onToggle={(id) =>
                  props.onDraft({
                    ...draft,
                    annualWheelEventIds: toggle(draft.annualWheelEventIds, id),
                  })
                }
          ...
          <div className="border-t border-line pt-5">
            ...
            <div className="mt-3 grid gap-4 sm:grid-cols-2">
              <Field label="Introduktion til rollen">
                <Textarea
                  value={draft.onboarding.introduction}
                  onChange={(e) =>
                    props.onDraft({
                      ...draft,
                      onboarding: {
              ...
              <Field label="Prioriterede opgaver de første 30 dage">
                <Textarea
                  value={draft.onboarding.first30Days}
                  onChange={(e) =>
                    props.onDraft({
                      ...draft,
                      onboarding: {
                        ...draft.onboarding,
                        first30Days: e.target.value,
                      },
                    })
                  }
                />
              </Field>
              <Field label="Praktisk information">
                <Textarea
                  value={draft.onboarding.practicalInformation}
                  onChange={(e) =>
                    props.onDraft({
                      ...draft,
                      onboarding: {
          ...
          <div className="flex justify-between gap-3">
            {draft.id ? (
              <Button
                onClick={() => props.onArchive(draft.id!)}
                type="button"
                variant="danger"
              >
                Arkivér
            ...
            <div className="flex gap-2">
              ...
              <Button disabled={props.saving} type="submit">
                {props.saving ? "Gemmer..." : "Gem jobkort"}
              </Button>
            </div>
          </div>
        </form>
  ...
  return (
    <section>
      <div className="flex items-center justify-between">
        <h3 className="font-semibold">Dokumenter og links</h3>
        <Button
          onClick={() =>
            onDraft({
              ...draft,
              documents: [...draft.documents, { title: "", url: "" }],
            })
          }
          size="sm"
          type="button"
...
function DynamicTemplates({
  ...
  return (
    <section>
      <div className="flex items-center justify-between">
        <h3 className="font-semibold">Opgaveskabeloner</h3>
        <Button
          onClick={() =>
            onDraft({
              ...draft,
              taskTemplates: [
                ...
                {
                  committeeId:
                    draft.committeeIds[0] || data.committees[0]?.id || "",
                  title: "",
                  description: "",
                  category: "",
      ...
      <div className="mt-3 space-y-4">
        {draft.taskTemplates.map((template, index) => (
          <div className="rounded border border-line p-3" key={index}>
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Titel">
                <Input
                  ...
                  onChange={(e) =>
                    onDraft({
                      ...draft,
                      taskTemplates: draft.taskTemplates.map((item, i) =>
                        i === index ? { ...item, title: e.target.value } : item,
                      ),
                    })
                  }
              ...
              <Field label="Udvalg">
                <select
                  className="field"
                  value={template.committeeId}
                  onChange={(e) =>
                    onDraft({
                      ...draft,
                      taskTemplates: draft.taskTemplates.map((item, i) =>
                        i === index
                          ? { ...item, committeeId: e.target.value }
                          : item,
                      ),
                    })
                  }
              ...
              <Field label="Deadline efter antal dage">
                <Input
                  min={0}
                  type="number"
                  value={template.defaultDeadlineDays ?? ""}
                  onChange={(e) =>
                    onDraft({
                      ...draft,
                      taskTemplates: draft.taskTemplates.map((item, i) =>
                        i === index
                          ? {
                              ...item,
                              defaultDeadlineDays: e.target.value
