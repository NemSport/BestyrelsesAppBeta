drop trigger if exists agenda_occurrences_keep_eventual_last
...
as $$
begin
  -- Agenda order is now controlled by explicit append/reorder flows.
  -- This trigger used to mutate NEW.position before insert and could reuse an
  -- occupied position when Eventuelt was not already the highest row.
  return new;
end;
$$;
...
begin
  ...
  if target_meeting_id is not null then
    ...
    raise log 'create_agenda_item next occurrence position: meeting_id=%, current_max=%, next_position=%',
      target_meeting_id,
...
begin
  ...
  raise log 'schedule_agenda_item next occurrence position: meeting_id=%, current_max=%, next_position=%',
    target_meeting_id,
    current_max_position,
    next_position;
